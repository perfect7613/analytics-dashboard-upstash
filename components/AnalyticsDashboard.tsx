...
const Badge = ({percentage}: {percentage: number}) => {
    ...
    const positiveClass = "bg-green-900/25 text-green-400 ring-green-400/25"
    const neutralClass = "bg-zinc-900/25 text-zinc-400 ring-zinc-400/25"
    const negativeClass = "bg-red-900/25 text-red-400 ring-red-400/25"

    return (
        <span className={`inline-flex gap-1 items-center rounded-md px-2 py-1 text-xs font-medium ring-1 ring-inset ${isPositive ? positiveClass : isNeutral ? neutralClass : negativeClass}` }>{isPositive ? <ArrowUpRight className="h-3 w-3"/> : null}{isNeutral ? <ArrowUpRight className="h-3 w-3"/> : null}{isNegative ? <ArrowDownRight className="h-3 w-3"/> : null}{percentage.toFixed(0)}%</span>
    )
}
  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <Card className="flex flex-col sm:grid grid-cols-4 gap-6">
            ...
            <div className="col-span-3 flex items-center justify-between flex-wrap gap-8">{topCountries?.map(([countryCode,number]) => {
        ...
        <Card>
            {timeseriesPageviews ? (
                ...
                } categories={["Visitors"]} index="name"/>
            ) : null}
        </Card>
    </div>
  )
